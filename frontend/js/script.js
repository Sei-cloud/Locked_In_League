...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (seasonSection) {
    seasonSection.style.display =
      localStorage.getItem("lockedInLeagueLastSection") === "season"
        ? ""
        : "none";
  }
  // --- Season Page Logic ---
  ...
  function renderSeasonPage() {
    ...
    seasonPageSelect.innerHTML = seasons
    ...
    const filteredTeams = teams.filter((t) => t.year == selected);
    ...
    if (filteredTeams.length === 0) {
      ...
      filteredTeams.forEach((team) => {
        html += `<li><strong>${team.name}</strong> (${
          team.city || ""
        }) - Year: ${team.year || ""} [${team.roster.length} players]</li>`;
      });
      html += "</ul>";
    }
  ...
  <form id="add-team-form" style="display:flex;flex-direction:column;gap:0.7em;max-width:350px;margin:auto;">
    <label style="display:flex;flex-direction:column;font-weight:500;">
      Team Name:
      <input name="name" required style="padding:0.4em;margin-top:0.2em;">
    </label>

    <label style="display:flex;flex-direction:column;font-weight:500;">
      Season Year:
      <input name="year" type="number" required style="padding:0.4em;margin-top:0.2em;">
    </label>
...
`;
  ...
  const API_BASE =
    window.location.hostname === "localhost" ? "http://localhost:4000" : ""; // adjust if hosted elsewhere
  ...
  async function updatePlayer(teamId, playerId, payload) {
    const res = await fetch(
      `${API_BASE}/api/teams/${teamId}/players/${playerId}`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      ...
      teamDiv.innerHTML = `
        ...
        <div class="team-roster" style="display:none;">
          <button class="add-player-btn" data-team="${
            team._id
          }" style="margin-bottom:0.7em;padding:0.6em 1.2em;background:#1976d2;color:#fff;border:none;border-radius:4px;font-size:0.95em;cursor:pointer;">Add Player</button>
          <button class="edit-team-btn" data-team="${
            team._id
          }" style="margin-left:0.5em;padding:0.6em 1.2em;background:#1976d2;color:#fff;border:none;border-radius:4px;font-size:0.95em;cursor:pointer;">Edit Team</button>
          <button class="delete-team-btn" data-team="${
            team._id
          }" style="margin-left:0.5em;padding:0.6em 1.2em;background:#e53935;color:#fff;border:none;border-radius:4px;font-size:0.95em;cursor:pointer;">Delete Team</button>
  ...
  function findTeamIndexById(id) {
    return teams.findIndex((t) => t._id === id);
  }
